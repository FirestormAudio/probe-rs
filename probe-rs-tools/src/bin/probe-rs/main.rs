...
impl Cli {
    async fn run(self, client: RpcClient, _config: Config, utc_offset: UtcOffset) -> Result<()> {
        ...
        match self.subcommand {
            ...
            Subcommand::ReadTrace(cmd) => cmd.run(&mut *client.registry().await, &lister),
...
enum Subcommand {
    ...
    #[clap(name = "itm")]
    Itm(cmd::itm::Cmd),
    /// Drain PTM instruction trace from the ETF circular buffer.
    #[clap(name = "read-trace")]
    ReadTrace(cmd::read_trace::Cmd),
